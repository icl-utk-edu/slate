...
#define fetch(A, m, n, bound) offs_d##A[min(ptrdiff_t(n*LD##A+m), bound)]
#define make_FloatingPoint(real, imag) as_scalar<T>(real)
#define conj(A)           (A)
#define add(A, B)         (A+B)
#define mul(A, B)         (A*B)
#define div(A, B)         (A/B)
    ...
    T alpha, T beta )
{
    ...
    int blx = 0; //blockIdx.x;   // block's m dimension
    int bly = 0; //blockIdx.y;   // block's n dimension
    ...
    for (kk = 0; kk < K-BLK_K; kk += BLK_K)
    {
        ...
        for (k = 0; k < BLK_K; k++)
        {
            ...
            for (n = 0; n < THR_N; n++) {
                ...
                for (m = 0; m < THR_M; m++) {
                    fma(op<CONJA>(rA[m]), op<CONJB>(rB[n]), rC[n][m]);
                    //fma(op<CONJA>(sA[k][m*DIM_X+idx]), op<CONJB>(sB[n*DIM_Y+idy][k]), rC[n][m]);
                }
            }
        }
