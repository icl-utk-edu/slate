...
/// If A contains trapezoid T, then the slice T = A[0:A.mb(), 0:A.mb()] is upper triangular,
/// Bi = Bi[0:B.mb(), 0:A.mb()]. Call trmm Bi = Bi T.
/// Dispatches to target implementations.
/// @ingroup he2hb_trmm_internal
///
template <Target target, typename scalar_t>
void he2hb_trmm(HermitianMatrix<scalar_t>&& AH, Matrix<scalar_t>&& A,
                Matrix<scalar_t>&& B,
                std::vector<int64_t>& indices,
                int priority, int64_t queue_index)
{
    he2hb_trmm(internal::TargetType<target>(),
               AH, A, B, indices, priority, queue_index);
}
