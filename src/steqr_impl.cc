...
///     On entry, the nrows-by-n matrix Z.
///     To compute eigenvectors of the tridiagonal matrix, Z should
///     contain the identity matrix.
///     To compute eigenvectors of the original Hermitian matrix, Z
///     should contain the unitary matrix used in the reduction to
///     tridiagonal form.
...
///     eigenvectors are desired, then ldz >= max( 1, nrows ).
///
/// @param[in] nrows
///     The number of rows of the matrix Z.
///     For distributed use in ScaLAPACK, this is typically
///     nrows = max( 1, numroc( n, nb, myprow, 0, nprocs ) ).
///
/// @param[out] work
    ...
    blas::real_type<scalar_t>* work, int64_t lwork )
{
    ...
    real_t safe_max = 1.0 / safe_min;
    real_t sqrt_safe_max = sqrt( safe_max ) / 3.0;
    real_t sqrt_safe_min = sqrt( safe_min ) / eps2;
