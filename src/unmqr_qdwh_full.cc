    ...
    #pragma omp master
    {
        ...
        for (int64_t k = k_begin; k != k_end; k += k_step) {
                             ...
                             depend(in:block[lastk])
            {
                ...
                if (side == Side::Left) {
                    C_trail = C.sub(k, i_end, 0, C_nt-1);
                    W_trail = W.sub(k, i_end, 0, C_nt-1);
                }
                else {
                    C_trail = C.sub(0, i_end, k, C_nt-1);
                    W_trail = W.sub(0, i_end, k, C_nt-1);
                }
...
template
void unmqr_qdwh_full< std::complex<float> >(
    Side side, Op op,
...
template
void unmqr_qdwh_full< std::complex<double> >(
    Side side, Op op,
