...
void test_ppotrf_work( Params& params, bool run )
{
    ...
    if ( params.check.value() == 'y' ) {
        ...
        int descB_ref[9];
        descinit_( descB_ref, &n, &s, &nb, &nb, &i0, &i0, &ictxt, &mloc, &info );
        assert( 0 == info );
...
void test_ppotrf( Params& params, bool run )
{
    switch (params.datatype.value()) {
        ...
        case libtest::DataType::Single:
            throw std::exception();// test_ppotrf_work< float >( params, run );
            break;

        case libtest::DataType::Double:
            test_ppotrf_work< double >( params, run );
            break;

        case libtest::DataType::SingleComplex:
            throw std::exception();// test_ppotrf_work< std::complex<float> >( params, run );
            break;

        case libtest::DataType::DoubleComplex:
            throw std::exception();// test_ppotrf_work< std::complex<double> >( params, run );
            break;
    }
}
