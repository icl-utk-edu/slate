...
void test_trmm_work( Params& params, bool run )
{
    ...
    // slate::Matrix<scalar_t> A_generalmatrix( n_, n_, &A_tst[0], local_lda, nb_, nb_, nprow, npcol, local_lda, MPI_COMM_WORLD );
    // slate::TriangularMatrix<scalar_t> A(uplo, A_generalmatrix);
    // int64_t local_ldb = (int64_t)descB_tst[8];
    // slate::Matrix<scalar_t> B( n_, n_, &B_tst[0], local_ldb, nb_, nb_, nprow, npcol, local_ldb, MPI_COMM_WORLD );
